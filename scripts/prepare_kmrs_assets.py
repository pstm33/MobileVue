...
PUBLIC_DIR = ROOT / "public" / "kmrs"
GENERATED_DIR = ROOT / "src" / "generated"
MANIFEST_FILE = GENERATED_DIR / "kmrs-assets.json"
...
def main():
    token = load_env().get("VITE_KMRS_API_TOKEN") or os.environ.get("VITE_KMRS_API_TOKEN")
    if not token:
        raise RuntimeError("VITE_KMRS_API_TOKEN is required in .env.local")

    manifest = build_manifest(collect_assets(token))
    print(f"Prepared {len(set(manifest.values()))} optimized KMRS images")
