    ...
    <template v-else>
      ...
      <article v-for="item in favourites" v-else :key="itemKey(item)" class="tagam-card favourite-card tap-motion overflow-hidden">
        <RouterLink class="grid grid-cols-[104px_1fr] gap-3 p-3" :to="itemLink(item)">
          ...
          <div class="min-w-0">
            <p class="brand-kicker m-0">{{ item.item_uuid ? text.dish : text.restaurant }}</p>
...
const labels = {
  ru: {
    title: "Избранное",
    refresh: "Обновить",
    hero: "Любимые рестораны и блюда",
    description: "Сохраняйте места и блюда, к которым хочется возвращаться.",
    dish: "Блюдо",
    restaurant: "Ресторан",
    remove: "Убрать из избранного",
    emptyTitle: "Избранного пока нет",
    emptyText: "Нажмите сердце у ресторана или блюда, и оно появится здесь.",
  },
  tk: {
    title: "Halanlarym",
    refresh: "Täzele",
    hero: "Halan restoranlar we tagamlar",
    description: "Gaýtadan dolanmak isleýän ýerleriňizi we tagamlaryňyzy saklaň.",
    dish: "Tagam",
    restaurant: "Restoran",
    remove: "Halanlarymdan aýyr",
    emptyTitle: "Halanlaryňyz ýok",
    emptyText: "Restoranda ýa-da tagamda ýürek nyşanyna basyň, ol şu ýerde peýda bolar.",
  },
  en: {
    title: "Favourites",
    refresh: "Refresh",
    hero: "Favourite restaurants and dishes",
    description: "Save places and dishes you want to come back to.",
    dish: "Dish",
    restaurant: "Restaurant",
    remove: "Remove from favourites",
    emptyTitle: "No favourites yet",
    emptyText: "Tap the heart on a restaurant or dish and it will appear here.",
  },
};
