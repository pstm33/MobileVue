    ...
    <template v-else>
      <div class="grid grid-cols-3 gap-3">
        <div class="soft-card p-3">
          <strong class="block text-xl">{{ results.restaurants.length }}</strong>
          <span class="muted text-xs">{{ app.copy.search.restaurants }}</span>
        </div>
        <div class="soft-card p-3">
          <strong class="block text-xl">{{ results.items.length }}</strong>
      ...
      <section v-if="results.restaurants.length" class="grid gap-3">
        <div class="section-title">
          <h2>{{ app.copy.search.restaurants }}</h2>
          <span class="muted text-sm">{{ results.restaurants.length }}</span>
        </div>

        <RouterLink
          v-for="(restaurant, index) in results.restaurants"
      ...
      <section v-if="results.items.length" class="grid gap-3">
        <div class="section-title">
          <h2>{{ app.copy.search.dishes }}</h2>
          <span class="muted text-sm">{{ results.items.length }}</span>
        </div>
      ...
      <div v-if="query && !results.restaurants.length && !results.items.length" class="soft-card p-5 text-center">
