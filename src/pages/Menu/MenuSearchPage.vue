<template>
  <q-header>
    <q-toolbar class="tagam-surface tagam-text-main">
      <q-btn
        @click="$router.back()"
        color="white"
        rounded
  ...
  <q-page class="tagam-menu-search-page">
    <q-space class="q-pa-xs tagam-surface-muted"></q-space>
        ...
        <div class="q-pa-md">
          <template v-if="hasData">
            <h5 class="no-margin text-weight-bold q-pb-sm">
              {{ $t("Search results for") }} вЂњ{{ q }}вЂќ ({{ totalFound }})
            </h5>
          </template>
          <template v-else>
            <h5 class="no-margin text-weight-bold">
              {{ $t("No results for") }} вЂњ{{ q }}вЂќ
            </h5>
            <p class="tagam-text-muted font13">
              {{
                $t(
