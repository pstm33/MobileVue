<template>
  <q-pull-to-refresh @refresh="refresh">
    ...
    <q-page
      padding
      class="q-pl-md q-pr-md tagam-order-success-page"
    ...
    >
      ...
      <template v-else>
        <div class="text-center q-mb-md">
          <div class="tagam-text-muted text-weight-600">
        ...
        <div class="row items-start justify-between">
          <div class="tagam-text-muted">{{ $t("Order #") }}</div>
          <div class="text-weight-bold">{{ order_info.order_id }}</div>
        </div>
        <div class="row items-start justify-between">
          <div class="tagam-text-muted">{{ $t("Order from") }}</div>
          <div class="text-weight-bold">{{ merchant.restaurant_name }}</div>
        </div>
        <div class="row items-start justify-between">
          <div class="tagam-text-muted col-2">{{ $t("Delivery") }}</div>
        ...
        <div class="row items-start justify-between">
          <div class="tagam-text-muted col text-weight-bold">{{ $t("Total") }}</div>
            ...
            <template v-slot:header>
              <q-item-section>
                <q-item-label>
                  ...
                  <span class="tagam-text-muted font13 q-ml-sm"
                    >({{ order_items.length }}
        ...
        <q-footer
          reveal
          class="q-pl-md q-pr-md q-pb-sm q-pt-sm tagam-text-main tagam-surface"
        >
          <q-btn
            unelevated
            no-caps
    ...
    orderDetails(done) {
      ...
      APIinterface.fetchDataByToken("orderDetails", {
        ...
        .then((data) => {
          ...
          if (!this.successHapticPlayed) {
            this.successHapticPlayed = true;
            this.haptics.successTriple();
          }
        })
        .catch((error) => {
          this.order_items = [];
