<template>
  <q-header reveal reveal-offset="50" class="tagam-surface">
    <q-toolbar>
      <q-btn
        @click="$router.back()"
        flat
  ...
  <q-page padding class="tagam-surface-muted tagam-address-select-page">
...
export default {
  ...
  computed: {
    mapReturnUrl() {
      const backUrl = APIinterface.empty(this.back_url) ? "/home" : this.back_url;
      return `/address/select?url=${encodeURIComponent(backUrl)}`;
    },
    encodedMapReturnUrl() {
      return encodeURIComponent(this.mapReturnUrl);
    },
  },
  methods: {
    getAddresses() {
