<template>
  ...
  >
    <q-toolbar class="myshadow-1">
      ...
      <template v-if="getChatWith">
        <q-avatar class="tagam-chat-header-avatar">
  ...
  <q-page class="flex items-stretch content-end q-pa-md tagam-account-chat-conversation-page">
    <div class="tagam-chat-thread">
      <q-inner-loading
        :showing="loading"
        size="md"
        color="primary"
        :label="$t('Please wait')"
        label-class="tagam-text-main"
        label-style="font-size: 1em"
        >
      </q-inner-loading>

      <template v-if="!loading && !hasMessageOld">
        <div class="tagam-chat-empty-thread">
          <div class="tagam-empty-icon">
            <q-icon name="eva-message-circle-outline" size="30px" />
          </div>
          <div class="tagam-title-md q-mb-xs">{{ $t("No messages yet") }}</div>
          <div class="tagam-body-muted">
            {{ $t("Write the first message") }}
          </div>
        </div>
      </template>
      ...
      <template v-for="items in getChatmessage" :key="items">
        <template v-if="items.messageType == 'audio'">
          <q-chat-message
            ...
            class="tagam-chat-bubble"
            :class="{ 'tagam-chat-bubble--sent': items.senderID == user_uuid }"
  ...
  <q-footer
    ...
    class="tagam-chat-composer tagam-surface tagam-text-main q-pl-sm q-pr-sm"
  >
        ...
        <template v-slot:append>
          <div class="q-gutter-sm">
            ...
            <q-btn
              ...
              :aria-label="hasMessage ? $t('Send message') : $t('Voice message')"
              no-caps
              size="md"
...
export default {
  ...
  computed: {
    ...
    hasChatDocID() {
      if (!APIinterface.empty(this.chating_with_user_uuid)) {
        return true;
      }
      return false;
    },
        ...
        (querySnapshot) => {
          ...
          querySnapshot.forEach((doc) => {
            if (doc.exists()) {
              ...
              let timestamp = this.resolveTimestamp(message.timestamp);
              this.data.push({
...
.tagam-account-chat-conversation-page {
  min-height: 100%;
  padding-bottom: 108px;
  background:
    radial-gradient(circle at 18% 0, rgba(255, 90, 47, 0.08), transparent 220px),
    linear-gradient(180deg, var(--tagam-bg-soft) 0, var(--tagam-bg) 320px);
}
...
.tagam-chat-header .q-toolbar {
  min-height: 68px;
  padding-left: 12px;
  padding-right: 14px;
}

.tagam-chat-header-avatar {
  width: 42px;
  height: 42px;
  margin-left: 8px;
...
.tagam-chat-bubble .q-message-text {
...
.tagam-chat-bubble.q-message-sent .q-message-text,
.tagam-chat-bubble--sent .q-message-text {
  border-radius: 20px 20px 8px 20px;
  border-color: rgba(255, 90, 47, 0.24);
  background: linear-gradient(135deg, var(--tagam-primary) 0%, #ff7a55 100%) !important;
  color: var(--tagam-primary-contrast) !important;
}

.tagam-chat-bubble .q-message-name,
.tagam-chat-bubble .q-message-stamp {
  color: var(--tagam-text-muted) !important;
}

.tagam-chat-bubble.q-message-sent .q-message-stamp {
...
.tagam-chat-input {
  ...
  background: var(--tagam-surface-elevated);
}

.tagam-chat-input .q-field__control {
  min-height: 56px;
}

.tagam-chat-input textarea {
  color: var(--tagam-text);
  font-weight: 650;
}

.tagam-chat-input textarea::placeholder {
  color: var(--tagam-text-subtle);
}

.tagam-chat-composer .q-uploader {
  width: 0;
  height: 0;
  min-height: 0;
  overflow: visible;
...
.tagam-chat-composer .q-uploader__list {
  position: absolute;
  left: 16px;
  right: 16px;
...
body.body--dark .tagam-chat-composer {
