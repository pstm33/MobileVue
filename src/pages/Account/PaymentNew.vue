<template>
  <q-pull-to-refresh @refresh="refresh">
    ...
    <q-page
      class="q-pl-md q-pr-md tagam-account-payment-new-page"
      :class="{ 'flex flex-center': !PaymentStore.hasData && !PaymentStore.loading2, 'row items-stretch': PaymentStore.hasData, 'tagam-surface-elevated text-white': $q.dark.mode, 'tagam-surface-muted tagam-text-main': !$q.dark.mode, }"
    >
      <template v-if="!PaymentStore.hasData && !PaymentStore.loading2">
        <div class="min-height-inherit flex flex-center">
          <div class="full-width text-center q-pb-xl">
            ...
            <p class="tagam-text-muted font12">
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <q-card
            flat
            class="radius8 col-12 tagam-account-payment-list-card"
          >
            <q-card-section class="q-gutter-y-sm">
              <div class="tagam-title-md">{{ $t("Payment Method") }}</div>
                  ...
                  >
                    <q-item-section avatar>
                      <template v-if="items.logo_type === 'icon'">
                        <div class="tagam-account-list-icon">
                      ...
                      <template v-else>
                        <q-img
                          ...
                          fit="contain"
                          class="tagam-account-payment-logo"
                        />
                      </template>
                    </q-item-section>
                    <q-item-section>
                      <q-item-label class="tagam-text-main text-weight-bold">
                        {{ displayPaymentName(items.payment_name) }}
                      </q-item-label>
                    </q-item-section>
                    <q-item-section side>
                      <q-icon name="chevron_right" class="tagam-text-muted" />
                    </q-item-section>
                  </q-item>
                </transition>
              </q-list>
