<template>
  ...
  >
    <q-card class="tagam-item-card rounded-borders-top">
      ...
      <div v-else>
        <q-card class="no-shadow">
          ...
          <q-card-section
            class="tagam-item-media-section no-wrap q-pa-none relative-position bn"
            style="
          ...
          <q-card-section>
            ...
            <TextComponents
              ...
              max_lenght="100"
              class_name="tagam-text-muted text-subtitle2 line-normal"
              :label="{
            ...
            <div v-if="cooking_data.length > 0" class="q-mb-sm">
              ...
              <div v-else class="tagam-text-muted font12 text-weight-medium q-mb-sm">
                {{ $t("Optional") }}
              </div>
            ...
            <div v-if="ingredients_data.length > 0" class="q-mb-sm">
              ...
              <div class="tagam-text-muted text-caption text-weight-medium q-mb-sm">
                {{ $t("Optional") }}
              </div>
            ...
            <template v-if="addons[item_size_id]">
              ...
              >
                <div class="q-mb-md">
                  <div
                    class="tagam-addon-group-header row justify-between items-center text-caption no-margin line-normal q-pa-sm radius8"
                  >
                    <div>
                    ...
                    <q-item-section :side="!sub_items.checked">
                      ...
                      >
                        <q-btn
                          ...
                          color="grey"
                          @click="handleAddonQtyChange(sub_items, -1)"
                        ></q-btn>
                          ...
                          <div v-else>
                            <q-btn
                              @click="handleAddonAdd(sub_items)"
                              round
                              unelevated
                              dense
                              size="11px"
              ...
              >
                <div class="col text-center">
                  <q-btn-group
                    unelevated
                    :class="[
                      'tagam-qty-stepper radius28 border-primary',
                      { 'tagam-item-qty-pulse': qtyPulse },
                    ]"
                  >
                    <q-btn
                ...
                <q-btn
                  ...
                  size="lg"
                  :class="[
                    'tagam-item-add-btn col-7 tagam-item-add-pulse',
                    { 'tagam-item-add-pulse--active': addPulse },
                  ]"
            ...
            <template v-else>
              <div
                class="tagam-item-footer fixed-bottom q-pa-sm border-grey-top1 shadow-1 row q-gutter-x-sm items-center"
              >
                <div class="col text-center">
                  <q-btn-group
                    unelevated
                ...
                <div class="col-6">
                  <q-btn
                    ...
                    color="primary"
                    :class="[
                      'tagam-item-add-btn fit tagam-item-add-pulse',
                      { 'tagam-item-add-pulse--active': addPulse },
                    ]"
                    no-caps
                    size="lg"
...
export default {
  ...
  setup() {
    ...
    return { CartStore, FavoriteStore, schedStore, haptics };
  },
  data() {
    return {
      item_dialog: false,
  ...
  methods: {
    ...
    decreaseItemQty() {
      if (this.item_qty > 1) {
        ...
        this.item_qty--;
        clearTimeout(this._qtyPulseTimer);
        this._qtyPulseTimer = setTimeout(() => {
          this.qtyPulse = false;
        }, 180);
      } else {
    ...
    handleAddonQtyChange(subItem, delta) {
      if (delta > 0) {
        if (subItem.disabled) {
          return;
        }
    ...
    handleAddonAdd(subItem) {
      if (subItem.disabled) {
        return;
      }
      this.haptics.impact("light");
      subItem.checked = true;
      subItem.qty = Math.max(subItem.qty || 1, 1);
    },
    resetData() {
      this.item_qty = 1;
      this.items = [];
    ...
    CheckaddCartItems() {
      this.addPulse = true;
      clearTimeout(this._addPulseTimer);
      this._addPulseTimer = setTimeout(() => {
        this.addPulse = false;
      }, 360);
    ...
    AddToCart() {
      ...
      const $data = {
        ...
        transaction_type:
          this.CartStore.geTransactiontype || this.schedStore.transaction_type,
        meta: $meta,
      ...
      APIinterface.AddToCart($data)
        .then((data) => {
          ...
          this.$emit("afterAdditems", data.details.cart_uuid, {
            image: this.image_featured ? this.image_featured : this.items.url_image,
            name: this.items.item_name,
          });
          this.item_dialog = false;
        })
        .catch((error) => {
