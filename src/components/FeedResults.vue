<template>
  <div v-if="done_fetch" class="tagam-section-title text-h6 text-weight-medium">
    {{ title }}
  </div>
  ...
  >
    <template v-slot:default>
      <template v-if="!hasMore && !hasData">
        <div
          class="text-subtitle2 tagam-text-muted text-center full-width q-pa-md"
  ...
  methods: {
    ...
    async fetchData(index, done) {
      try {
        ...
        const feedData = Array.isArray(details.data) ? details.data : [];
        ...
        this.current_page++;
        this.data = [...this.data, ...feedData];

        if (details.is_last_page) {
          this.hasMore = false;
          done(true);
          return;
        }
...
.tagam-restaurant-list-item:hover
  transform: translateY(-1px)

.tagam-restaurant-list-item :deep(.tagam-restaurant-feed-card)
  margin-bottom: 0

.tagam-restaurant-list-item :deep(.q-focus-helper)
  border-radius: 26px
</style>
